...
Page({
  ...
  getVersion:async function() {
    ...
    if(res.result.showLottery){
    }
    that.setData({
      iconList:res.result.iconList,
      config: res.result
    })
  },
  copyUrl:async function(e){
  ...
  getDetail: async function (blogId) {
    wx.showLoading({
      ...
      console.info('postDetail',postDetail.sourceFrom)
      if(postDetail.sourceFrom== 'haokan'){
        that.setData({
          post: postDetail
        })
        console.info(that.data.post)
      }else{
        console.log('111')
        let content = app.towxml(postDetail.content, 'html');
 ...
 */
  ...
  */
  ...
  getProportion:async function(e){
    var that = this
    console.log(e)
    ...
    var h = windowInfo.windowWidth*(a)
